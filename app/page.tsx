...
import { Users, Volume2, VolumeX, Share2, Heart, Eye, Clock, History, Search, Loader2, Maximize2, PlayCircle, PauseCircle } from "lucide-react";
...
export default function Home() {
  ...
  const [selectedQuality, setSelectedQuality] = useState<"origin" | "uhd_60" | "hd_60" | "hd" | "sd" | "ld">("hd");
  ...
  const getOptimalQuality = (qualities: TikTokResponse['live']['streamQualities'], networkSpeed: number) => {
    ...
    const availableQualities = Object.entries(qualities)
      ...
      .sort(([a], [b]) => qualityLevels[a as keyof typeof qualityLevels] - qualityLevels[b as keyof typeof qualityLevels]);
  ...
  const checkAndUpdateQuality = async () => {
    ...
    const optimalQuality = getOptimalQuality(streamData.live.streamQualities, speed);
    if (optimalQuality && optimalQuality !== selectedQuality) {
      setSelectedQuality(optimalQuality as any);
    }
    setLoadingQuality(false);

    qualityCheckTimeoutRef.current = setTimeout(checkAndUpdateQuality, 30000);
  };
  ...
  const fetchStreamData = async (searchUsername: string = username) => {
    ...
    try {
      const response = await fetch(`https://tiktok-live-stream.vercel.app/api/tiktok-live/${searchUsername}`);
      ...
      if (data.live.isLive) {
        refreshIntervalRef.current = setInterval(async () => {
          try {
            const refreshResponse = await fetch(`https://tiktok-live-stream.vercel.app/api/tiktok-live/${searchUsername}`);
            const refreshData: TikTokResponse = await refreshResponse.json();
            setStreamData(prev => prev ? {
              ...prev,
              live: {
                ...prev.live,
                viewerCount: refreshData.live.viewerCount
              }
            } : refreshData);
          } catch (err) {
  ...
  useEffect(() => {
    ...
    const cleanupPlayers = () => {
      ...
      if (flvPlayerRef.current) {
        try {
          if (flvPlayerRef.current.destroy && typeof flvPlayerRef.current.destroy === 'function') {
        ...
        if (window.Hls.isSupported()) {
          ...
          hls.on(window.Hls.Events.ERROR, (event, data) => {
            if (data.fatal) {
              ...
              if (autoQuality) {
                const qualities = Object.keys(streamData.live.streamQualities || {});
                const currentIndex = qualities.indexOf(selectedQuality);
                if (currentIndex > 0) {
                  setSelectedQuality(qualities[currentIndex - 1] as any);
                }
              }
            }
          });
  ...
  const getStreamUrl = () => {
    if (!streamData?.live.streamQualities?.[selectedQuality]?.urls) return null;
            ...
            <Card className="overflow-hidden bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-2 shadow-xl transform hover:scale-[1.01] transition-all duration-300">
              {streamData.live.isLive && getStreamUrl() && (
                <div ref={videoContainerRef} className="relative aspect-video bg-black group">
                  ...
                  />
                  <div className={`absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent transition-opacity duration-300 ${showControls || !isFullscreen ? 'opacity-0 group-hover:opacity-100' : 'opacity-0'}`} />
                  <div className={`absolute bottom-0 left-0 right-0 p-4 flex items-center gap-4 transition-all duration-300 transform ${showControls || !isFullscreen ? 'opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0' : 'opacity-0 translate-y-full'}`}>
              ...
              <div className="p-8 space-y-6">
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-muted-foreground text-lg mb-2">@{streamData.user.uniqueId}</p>
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2">
                        <Users className="w-5 h-5 text-violet-500 dark:text-violet-400" />
                        <span className="text-lg">{streamData.user.followers.toLocaleString()} followers</span>
                      </div>
                      <span className="text-muted-foreground text-lg">
                        {streamData.user.following.toLocaleString()} following
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Tooltip content="Like">
                      <Button
                        ...
                        className={`transition-all duration-300 hover:scale-110 ${
                          isLiked ? 'bg-pink-500 text-white border-pink-500 hover:bg-pink-600 hover:border-pink-600 dark:bg-pink-400 dark:border-pink-400' : ''
                        }`}
                      >
                ...
                <div className="border-t border-border/50 pt-6 dark:border-gray-700">
                  <div className="flex items-center gap-3 mb-4">
                    {streamData.live.isLive ? (
                      <>
                        ...
                        <span className="font-medium text-lg">LIVE NOW</span>
                        <div className="flex items-center gap-4 text-muted-foreground text-lg">
                          <div className="flex items-center gap-2">
                            ...
                            <span>{streamData.live.viewerCount?.toLocaleString()} watching</span>
                          </div>
                          {streamData.live.startTime && (
                            <div className="flex items-center gap-2">
                              <Clock className="w-5 h-5 text-violet-500 dark:text-violet-400" />
                              <span>Started at {formatTime(streamData.live.startTime)}</span>
                            </div>
                          )}
                        </div>
                      </>
                      ...
                      <>
                        ...
                        <span className="text-muted-foreground text-lg">Not Live</span>
                      </>
                    )}
                  </div>
                  
                  {streamData.live.isLive && streamData.live.title && (
                  ...
                  {streamData.live.isLive && streamData.live.streamQualities && (
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          id="auto-quality"
                        />
                        <Label htmlFor="auto-quality" className="text-sm">
                          Auto Quality {loadingQuality && <Loader2 className="inline-block w-4 h-4 animate-spin ml-2" />}
                        </Label>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(streamData.live.streamQualities)
                          .filter(([quality]) => quality !== 'ao')
                          .map(([quality, data]) => (
                            <Button
                              key={quality}
                            ...
                            >
                              {getQualityLabel(quality, data.resolution)}
                            </Button>
                          ))}
                      </div>
                    </div>
